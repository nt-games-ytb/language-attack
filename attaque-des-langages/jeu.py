...
joueur_actuel = joueur.Joueur("", 100, 100, 0, 0, 0, 1, 0, ["print"], 1, 0.05, os.path.dirname(os.path.realpath(__file__)), "\sauvegarde.xml")
...
if jeu.fichier_existe("sauvegarde.xml") == False: #Si il n'y a pas de sauvegarde
    ...
    jeu.creation_de_joueur(joueur_actuel, village_utilisé)
    initialisation.saut_de_lignes()
    initialisation.scenario(village_utilisé)
    initialisation.saut_de_lignes()
    initialisation.demande_tutoriel()
else:
    jeu.afficher_les_joueurs("\sauvegarde.xml")
    jeu.chargement_du_joueur("\sauvegarde.xml", joueur_actuel, village_utilisé, int(input("Quelle option choisis-tu ? ")))
